...
#       done  >>  $LOGF_TRG
       ...
       cat $LOGF_HEAD $LOGF_TRG | $BASEDIR/send_msg.sh $CONFIG $SCRIPT_NAME $HOST $DB "TBS usage warning: free space too low current: ${CUR_VAL}%, threshold: ${limPER}%, limGB: ${limGB}"
