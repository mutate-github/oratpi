    ...
    awk '!/LAG_MINUTES/ && !/--------/ && !/^ *$/{print $2" "$(NF-1)" "$NF}' $LOG_FILE | while read DEST_ID SEQ_GAP_NOW LAG_MINUTES_NOW; do
      ...
      if [ -s $TRG_FILE_SEQ_GAP ]; then
        if [ "$SEQ_GAP_NOW" -lt "$SEQ_GAP" ]; then
          ...
          rm $TRG_FILE_SEQ_GAP
          cat $LOG_FILE | $BASEDIR/send_msg.sh $CONFIG $0 $HOST $DB "- RECOVER: $(date +%H:%M:%S-%d/%m/%y) was ${SEQ_GAP_WAS}, now ${SEQ_GAP_NOW} archivelogs not applyed to standby dest_id: ${DEST_ID} (SEQ_GAP limit = $SEQ_GAP logs)"
          echo "SEQ_GAP recover host: "${HOST} " database: "${DB}
        fi
      else
        if [ "$SEQ_GAP_NOW" -ge "$SEQ_GAP" ]; then
          echo "$SEQ_GAP_NOW" > "$TRG_FILE_SEQ_GAP"
          cat $LOG_FILE | $BASEDIR/send_msg.sh $CONFIG $0 $HOST $DB "- TRIGGER: $(date +%H:%M:%S-%d/%m/%y) now ${SEQ_GAP_NOW} archivelogs not applyed to standby dest_id: ${DEST_ID} (SEQ_GAP limit = $SEQ_GAP logs)"
      ...
      if [ -s $TRG_FILE_LAG_MINUTES ]; then
        if [ "$LAG_MINUTES_NOW" -lt "$LAG_MINUTES" ]; then
          ...
          cat $LOG_FILE | $BASEDIR/send_msg.sh $CONFIG $0 $HOST $DB "- RECOVER: $(date +%H:%M:%S-%d/%m/%y) standby dest_id: ${DEST_ID} was ${LAG_MINUTES_WAS}, now ${LAG_MINUTES_NOW} minuted behind (LAG_MINUTES limit = $LAG_MINUTES min)"
        ...
        if [ "$LAG_MINUTES_NOW" -ge "$LAG_MINUTES" ]; then
          ...
          cat $LOG_FILE | $BASEDIR/send_msg.sh $CONFIG $0 $HOST $DB "- TRIGGER: $(date +%H:%M:%S-%d/%m/%y) standby dest_id: ${DEST_ID} is ${LAG_MINUTES_NOW} minutes behind (LAG_MINUTES limit = $LAG_MINUTES min)"
      ...
      "${REPEAT_AT}")
         ...
         if [ "$FF" -eq 1 ]; then
           ...
           cat $LOG_FILE | $BASEDIR/send_msg.sh $CONFIG $0 $HOST $DB "- TRIGGER REPEAT: $(date +%H:%M:%S-%d/%m/%y) more ${SEQ_GAP_NOW} archivelogs not applyed to standby dest_id: ${DEST_ID} (SEQ_GAP limit = $SEQ_GAP logs)"
         ...
         if [ "$FF" -eq 1 ]; then
           ...
           cat $LOG_FILE | $BASEDIR/send_msg.sh $CONFIG $0 $HOST $DB "- TRIGGER REPEAT: $(date +%H:%M:%S-%d/%m/%y) standby dest_id: ${DEST_ID} more ${LAG_MINUTES_NOW} minutes behind (LAG_MINUTES limit = $LAG_MINUTES min)"
