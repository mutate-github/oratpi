...
Полезна для небольших компаний с БД Oracle, без дорогого инстумента администрирования. Мониторинг легко справляется с 100-120 instances Oracle DB.
Необходим доступ по ssh (предпочтительно) или TNS. (Также можно комбинировать ssh и TNS доступ в скрипте правил доступа rtpi)

Ключевые преимущества oratpi:
- Все-in-One: Замена десяткам разрозненных скриптов.
- Консольный CLI интерфейс: Быстрый запуск и интеграция в инфраструктуру серверов БД.
...
1. Скрипты диагностики БД Oracle. Скрипты: rtpi, tpi. Документация doc/doc_tpi.txt
2. Скрипты мониторинга БД Oracle. Скрипты: mon_all.sh, mon_*.sh. Документация doc/doc_mon.txt
3. Скрипты первичного аудита БД Oracle. Скрипт: audit_tpi.sh. Документация doc/doc_aud.txt
...
```
Usage: /home/t.mukhametshin/start/tpi/tpi <DBSID/PDB> [SPID\SID\OS_client_PID] [a|in|k] [p|ph [FALSE] <param>] [license] [services] [dir] [resource_limit] [resumable] | db | analyze | audit | health | sga | pga | size | arch | redo | undo | sesstat | sysstat | segstat | o . | s . | e . | t . | i . | l . | c . | u . | r. | trg . | profile | links | latch | lock | bind | pipe | longops | scheduler | job | rman | get_ddl | trace | kill | exec | alert | report ash/awr | corrupt | sql | ash | dhash | oratop | spm
...
p [FALSE] [PAR1 PAR2 ..] | ph [FALSE] [PARAMETER] | license | services | dir | acl | resource_limit | resumable - instance parameters or hidden parameters, [FALSE] - only changed parameters, gv\license - HWM sessions, gv$services, dba_directories, dba_network_acls, gv$resource_limit
db [ nls|option|properties|fusage ] - gv$instance, v$database, dba_registry, dba_registry_sqlpatch, nls_database_paramters, v$option, database_properties information
...
audit [logons [dd/mm/yy-HH:MI-HH:MI(hours)] | maxcon [dd/mm/yy-HH:MI-HH:MI(hours)] [CNT] | 1017 [dd/mm/yy-HH:MI-HH:MI(hours)] [USR] | obj [dd/mm/yy-HH:MI-HH:MI(hours)] [OBJ] ] - DDL users audit, logons - all logon, maxcon - max of db connections above CNT (def 100) for a period (def 1H), 1017 - failed login, obj - obj audit
health [in=INST_ID] [con=CON_ID] [cr | hot] - Database health parameters (Hit Ratio / Get Misses cache, System Events Waits, Consistent Read buffers in SGA | Hot buffers)
sga - SGA information
...
undo [recovery | tuning] - undo active transaction information, recovery information, tuning MAXQUERYLEN from DBA_HIST_UNDOSTAT
...
o OBJECT_NAME | OBJECT_ID | invalid [OWNER] | ddl [last N hours] | depend REFERENCED_OBJ_NAME | depend last_analyzed HH OBJ_NAME [OWNER] - Show what REFERENCED_OBJ_NAME was last analyzed in depended OBJ_NAME - dba_objects information
s SEGMENT_NAME [OWNER] | nonsystem - dba_segments information, nonsystem - NON-SYSTEM objects detected in SYSTEM tablespace
e SEGMENT_NAME [OWNER] - dba_extents, dba_tablespaces, dba_data_files - Extents in datafiles information
t [part] TABLE_NAME [OWNER] | last_analyzed HH [OWNER] | chained [PERCENT] | modifications [TABLE_NAME [OWNER]] - dba_tables, dba_part_tables, dba_tab_partitions, dba_tab_subpartitions, dba_tab_modifications information
mv - dba_mviews, dba_mview_logs, dba_refresh_children matview information
...
l [LOB_NAME] | [table TABLE OWNER COLUMN_NAME] - dba_lobs information, table - show actual size of LOB segmsnt and unused segment information and cmd for rebuld -- sum(dbms_lob.getlength(CLOB_COL)/1024/1024)
...
u [ USERNAME [{dba_sys_privs|dba_tab_privs|dba_role_privs} PRIVILEGE|OBJECT|ROLE] ] - dba_users - for USER or ROLE information from dba_sys_privs, dba_tab_privs, dba_role_privs view for PRIVILEGE|OBJECT|ROLE
r [ {role_sys_privs|role_tab_privs|role_role_privs} ROLE PRIVILEGE|OBJECT|ROLE ] - role_sys_privs role_tab_privs role_role_privs information for ROLE and PRIVILEGE|OBJECT|ROLE
...
links [LINK_NAME | wholink] - links information, wholink - who is querying via dblink. The GTXID will match for both databases
...
bind [ SQL | SQL_ID ] - V$SQL_BIND_CAPTURE for SQL_ID or sql not using bind variable information
...
job [run] [JOB_ID|SID|SPID|OWNER] - dba_jobs, dba_jobs_running information
...
kill [SID SERIAL INST_ID] | [idle NUM_HOURS] | ["where S.USERNAME='SomeUser'"] | [ zombie_unix_list | zombie_win_list ] - Kill session, idle 5 - kill idle sessions for 5 hours and more, zombie_unix_list - only show zombie
...
report [ash {text|html} -60] -for last hour, [awr {text|html} dd/mm/yy-HH:MI-HH:MI(hours)], [awrdd {text|html} dd1/mm/yy-HH:MI-HH:MI(hours) dd2/mm/yy-HH:MI-HH:MI(hours)], [awrsql {text|html} dd/mm/yy-HH:MI-HH:MI(hours) sql_id], [addm text dd/mm/yy-HH:MI-HH:MI(hours)] - oracle reports
...
sql [ [top] | [SQL_ID | SQLTEXT] | [hv HASH_VALUE] | [plan SQL_ID] | [expand SQL_ID] | [sqlarea_vercnt] | [child_reason SQL_ID] | sql_bind_metadata SQL_ID | sql_shared_cursor SQL_ID | [sqlstat [SQL_ID|par|inv|fch|sor|exe|pio|lio|row|cpu|ela|iow|mem] [executions] ] - Find out sql_id by SQLTEXT\SQL_ID from V$SQL, plan from VSQL_PLAN by sql_id, sqlstat from V$SQLSTAT by sql_id, GV$\SQLAREA
ash [in=INST_ID] [con=CON_ID] [dd/mm/yy-HH:MI-HH:MI(hours) - def1h] [ event | sess [SID SERIAL# [all|nosqlid|tchcnt] | SQL_ID] | where ["FIELD CONDITIONS"] | sql [top [pga [sess]|EVENT]] [all [EVENT]] [SQL_ID|SQL_TEXT] | plan SQL_ID [fmt display plan] | sqlstat [SQL_ID|par|inv|fch|sor|exe|pio|lio|row|cpu|ela|iow|mem] [executions] | insection [username|service|machine|program,module,action|sql_opname,sql_plan_operation,sql_plan_options|PLSQL_ENTRY_OBJECT_ID|event|wait_class] | waitchains ["username||':'||program2||':'||sql_opname||':'||event2" "event2 like 'enq: TX %'"] | temp [sizeMb] | (umc)chart ] - Top SQL, Events, Sessions GV$ACTIVE_SESSION_HISTORY
dhash [in=INST_ID] [con=CON_ID] [dd/mm/yy-HH:MI-HH:MI(hours) - def1h] [ event | sess [SID SERIAL# [all|nosqlid|tchcnt] | SQL_ID] | where ["FIELD CONDITIONS"] | sql [top [pga [sess]|EVENT]] [all [EVENT]] [SQL_ID|SQL_TEXT] | plan SQL_ID PHV [fmt display plan] | sqlstat [SQL_ID|pio|lio|cpu|exe|ela|fch|sor|iow|row] [executions] | cmpsql [PERCENT | dd/mm/yy-HH:MI-HH:MI [PERCENT]] | insection [username|service|machine|program,module,action|sql_opname,sql_plan_operation,sql_plan_options|PLSQL_ENTRY_OBJECT_ID|event|wait_class] | waitchains ["username||':'||program2||':'||sql_opname||':'||event2" "event2 like 'enq: TX %'"] | growseg [TBS] [SEGMENT] | segstat [SEGMENT] [OWNER] [SORT_COL] | temp [sizeMb] | (umc)chart | iostat [df|redo|ctl|temp|arch|other] ] | awrinfo - Top SQL, Events, Sessions DBA_HIST_ACTIVE_SESS_HISTORY
