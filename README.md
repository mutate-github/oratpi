...
```
Usage: /home/oracle/start/tpi/tpi <DBSID/PDB> [SPID\SID\OS_client_PID] [a|in|k] [p|ph [FALSE] <param>] [services] [dir] | lock | db | analyze | audit | health | oratop | sga | pga | size | arch | redo | undo | sesstat | segstat | o . | s . | e . | t . | i . | l . | c . | u . | r. | trg . | profile | links | latch | bind | pipe | longops | scheduler | job | rman | get_ddl | trace | kill | exec | alert | report ash/awr | corrupt | sql | ash | dhash | spm
