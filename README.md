...
```
Usage: /cygdrive/c/work/scripts/tpi/tpi <DBSID/PDB> [SPID\SID\OS_client_PID] [a|in|k] [p|ph [FALSE] <param>] [license] [services] [dir] [resource_limit] [resumable] | db | analyze | audit | health | sga | pga | size | arch | redo | undo | sesstat | sysstat | segstat | o . | s . | e . | t . | i . | l . | c . | u . | r. | trg . | profile | links | latch | lock | bind | pipe | longops | scheduler | job | rman | get_ddl | trace | kill | exec | alert | report ash/awr | corrupt | sql | ash | dhash | oratop | spm
...
p [FALSE] [PAR1 PAR2 ..] | ph [FALSE] [PARAMETER] | license | services | dir | resource_limit | resumable - instance parameters or hidden parameters, [FALSE] - only changed parameters, gv\license - HWM sessions, gv$services, dba_directories, gv$resource_limit
...
l [LOB_NAME] | [table OWNER TABLE COLUMN_NAME] - dba_lobs information, table - show actual size of LOB segmsnt and unused segment information and cmd for rebuld -- sum(dbms_lob.getlength(CLOB_COL)/1024/1024)
...
ash [in=INST_ID] [con=CON_ID] [dd/mm/yy-HH:MI-HH:MI(hours) - def1h] [ event | sess [SID SERIAL# [all|nosqlid|tchcnt] | SQL_ID] | where [FIELD CONDITION] | sql [top [pga [sess]|EVENT]] [all [EVENT]] [SQL_ID|SQL_TEXT] | plan SQL_ID [fmt display plan] | sqlstat [SQL_ID|par|inv|fch|sor|exe|pio|lio|row|cpu|ela|iow|mem] [executions] | insection [username|service|machine|program,module,action|sql_opname,sql_plan_operation,sql_plan_options|PLSQL_ENTRY_OBJECT_ID|event|wait_class] | waitchains ["username||':'||program2||':'||sql_opname||':'||event2" "event2 like 'enq: TX %'"] | temp [sizeMb] | (umc)chart ] - Top SQL, Events, Sessions GV$ACTIVE_SESSION_HISTORY escape \'
dhash [in=INST_ID] [con=CON_ID] [dd/mm/yy-HH:MI-HH:MI(hours) - def1h] [ event | sess [SID SERIAL# [all|nosqlid|tchcnt] | SQL_ID] | where [FIELD CONDITION] | sql [top [pga [sess]|EVENT]] [all [EVENT]] [SQL_ID|SQL_TEXT] | plan SQL_ID PHV [fmt display plan] | sqlstat [SQL_ID|pio|lio|cpu|exe|ela|fch|sor|iow|row] [executions] | cmpsql [PERCENT | dd/mm/yy-HH:MI-HH:MI [PERCENT]] | | insection [username|service|machine|program,module,action|sql_opname,sql_plan_operation,sql_plan_options|PLSQL_ENTRY_OBJECT_ID|event|wait_class] | waitchains ["username||':'||program2||':'||sql_opname||':'||event2" "event2 like 'enq: TX %'"] | growseg [TBS] [SEGMENT] | segstat [SEGMENT] [OWNER] [SORT_COL] | temp [sizeMb] | (umc)chart | iostat [df|redo|ctl|temp|arch|other] ] | awrinfo - Top SQL, Events, Sessions DBA_HIST_ACTIVE_SESS_HISTORY esape \'
