...
#-- end skip for host
  ...
  case "$OS" in
   ...
   AIX)   $SSHCMD "$HOST" "lsps -s | tail +2  | cut -d% -f1 | awk '{printf \$2}'" > $LOGF
          PCT=$(head -1 $LOGF)
          ;;
   SunOS) # virtual limit  $SSHCMD "$HOST" "used=\$(/usr/sbin/swap -s | awk '{print \$9}' | tr -d 'k'); avail=\$(/usr/sbin/swap -s | awk '{print \$11}' | tr -d 'k'); total=\$((used + avail)); percent=\$(( used * 100 / total )); echo -n \$percent" > $LOGF
          $SSHCMD "$HOST" "/usr/sbin/swap -l | awk 'NR > 1 { total += \$4; free += \$5 } END { if (total>0) { percent = (total-free) * 100 / total ; printf \"%.0f\", percent } }'" > $LOGF
          PCT=$(head -1 $LOGF)
          ;;
  esac
  echo "PCT: "$PCT
